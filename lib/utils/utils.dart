...
Future<int> getMaxAvailableTimeSlot(DateTime dt) async {
  ...
  // Se sincronizează timpul cu serverul

  DateTime syncTime = now.add(Duration(milliseconds: offset));

  // Se compară timpul sincronizat cu timpul local pentru a activa slotul de timp
  // și returnează următorul slot disponibil
  ...
  if (syncTime.isBefore(DateTime(now.year, now.month, now.day, 9, 0)))
    ...
    return 19;
  else if (syncTime.isAfter(DateTime(now.year, now.month, now.day, 18, 3)) &&
